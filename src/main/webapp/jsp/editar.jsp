...
<head>
    <meta charset="UTF-8">
    <title>Editar Item: <c:out value="${item.titulo}"/></title>
    <style>
...
<body>
    <h1>Editar Item de Mídia: <c:out value="${item.titulo}"/></h1>
...
<!--
Função do arquivo:
Esta página JSP contém o formulário para editar um item de mídia existente.
O formulário é pré-preenchido com os dados do objeto `item` usando JSTL/EL.
O ID do item é enviado como um campo oculto (`hidden`) para que o Servlet saiba qual item atualizar.
O formulário envia os dados via POST para o Servlet /editar.
-->
